...
namespace Commercial.Models
{
    public class BesoinService
    {
        ...
        public void UpdateEtatProduit(int idproduit,int idresponsable)
        {
            ConnexionBDD connexion = new ConnexionBDD();

            using (NpgsqlConnection connection = connexion.Connect())
            {
                string sql = "UPDATE besoin_service SET etat = 11 FROM service WHERE besoin_service.idproduit = @idproduit AND service.idresponsable = @idresponsable AND service.idservice = besoin_service.idservice;";
